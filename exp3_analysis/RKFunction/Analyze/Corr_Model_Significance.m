function [corrA,corrB,correlationDiffs] = Corr_Model_Significance(dataA,dataB,Model,bootstrap)
...
corrB = zeros(numBootstrap,1);

for i = 1:numBootstrap
    % === 1. データAでの基準相関係数を計算 ===
    % 1.a.b 条件ごとにブートストラップで平均を計算
    sampleA = MeanArray(dataA,illumDim);

    % 1.c 平均値データ間の相関係数
    corrA(i) = corr(sampleA(:),Model);
    ...
    % 2.b データAとデータB間の相関係数
    corrB(i) = corr(sampleB(:),Model);

    % === 3. 相関係数の差を計算 ===
    correlationDiffs(i) = corrA(i) - corrB(i);
end
end
