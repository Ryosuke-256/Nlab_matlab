function [] = Make_2dims_vsgraph_Coef_Hist(array1,err_array1,array2,err_array2,Filename,labelname1,labelname2,amp)
resultpath = "./results/";
ana_result = "analysis/";
Datapath = "./data/";
...
MatNames4 = {'cu.0.025', 'cu.0.129', 'pla.0.075', 'pla.0.225'};
...
for idx1 = 1:size(array1,2)
    ...
    xlabel(labelname1,'FontSize',12*amp);
    ylabel(labelname2,'FontSize',12*amp);
    title(sprintf('%s: \nCoef = %.2f,Slope = %.2f', string(MatNames4(idx1)),coef_list(idx1),slop_list(idx1)),'FontSize',12*amp);
    grid on;
end
...
for idx1 = 1:size(array1,2)
    ...
    title(sprintf('%s: \nCoef = %.2f,Slope = %.2f', string(MatNames4(idx1)),coef_list(idx1),slop_list(idx1)),'FontSize',12*amp);
    legend([bar_exp3, bar_old], {labelname1, labelname2}, 'Location', 'southeast','Orientation','vertical');
    legend('boxoff');
    grid on;
end
