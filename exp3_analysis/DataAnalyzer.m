classdef DataAnalyzer < handle
    ...
    methods (Access = private)
            ...
            try
                switch plotOptions.Mode
                    ...
                    case {"HM", "HS"}
                        ...
                        ceiling_distAB_list = zeros(plotOptions.Bootstrap,loopLimit);
                        p_value_list = zeros(loopLimit);
                        observed_corr_list = zeros(loopLimit);
        ...
        %% Bootstrap
        function generateBootstrapPlot_ver2(obj, plotData, plotOptions)
            dataA = plotData.targetA;
            dataB = plotData.targetB;
            ...
            try
                switch plotOptions.Mode
                    case "H"
                        ...
                        % === ステップ1: 観測された実データの相関係数を計算 ===
                        patternA = obj.MeanArray(dataA,1);
                        patternB = obj.MeanArray(dataB,1);
                        observed_corr = corr(patternA, patternB);

                        % === ステップ2: データAのノイズ天井を推定 ===
                        noise_ceiling_dist_A = estimateNoiseCeiling(dataA_r, plotOptions.Bootstrap, plotOptions.Split);

                        % === ステップ3: p値の算出と結論 ===
                        p_value = sum(noise_ceiling_dist_A <= observed_corr) / length(noise_ceiling_dist_A);

                        % === ステップ4: 結果の表示と可視化 ===
                        fprintf('\n観測された相関 corr(A, B): %.4f\n', observed_corr);
                        fprintf('データAのノイズ天井の平均: %.4f\n', mean(noise_ceiling_dist_A));
                        fprintf('p値: %.4f\n', p_value);
                        if p_value > 0.05
                        ...
                        % 可視化
                        fig = figure('Visible','off');
                        plotNoiseCeiling(observed_corr, noise_ceiling_dist_A, p_value);
                        title(sprintf('%s vs %s %s', plotOptions.NameA, plotOptions.NameB, plotOptions.Property), 'FontSize', 18*plotOptions.Amp);
                    ...
                    case {"HM", "HS"}
                        ...
                        loopLimit = length(labels);
                        %[corrA, corrAB, ~, threshold] = Corr_Significance_MS(dataA_r, dataB_r, plotOptions.Bootstrap, loopLimit);
                        for i = 1:loopLimit
                            % === ステップ1: 観測された実データの相関係数を計算 ===
                            patternA = obj.MeanArray(dataA,2);
                            patternB = obj.MeanArray(dataB,2);
                            observed_corr = corr(patternA(:,i), patternB(:,i));

                            % === ステップ2: データAのノイズ天井を推定 ===
                            noise_ceiling_dist_A = estimateNoiseCeiling(squeeze(dataA_r(:,i)), plotOptions.Bootstrap, plotOptions.Split);

                            % === ステップ3: p値の算出と結論 ===
                            p_value = sum(noise_ceiling_dist_A <= observed_corr) / length(noise_ceiling_dist_A);

                            % === ステップ4: 結果の表示と可視化 ===
                            fprintf('%sの結果',labels(i));
                            fprintf('実験のcorr(A, B): %.4f\n', observed_corr);
                            fprintf('データAのノイズ天井の平均: %.4f\n', mean(noise_ceiling_dist_A));
                            fprintf('p値: %.4f\n', p_value);
                            if p_value > 0.05
                                disp('結論: corr(A,B)はノイズ天井の範囲内です。\n');
