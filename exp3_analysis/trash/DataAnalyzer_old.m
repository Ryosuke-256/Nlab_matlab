classdef DataAnalyzer < handle
    ...
    methods
        ...
        function plotCorrBootstrap(obj,dataSpecA,dataSpecB,options)
            ...
            if options.Mode == 1
                graphtitle = sprintf('%s vs %s about %s\nCorrelaton Coefficient', dataSpecA.SetName, dataSpecB.SetName, options.Property);
                
                % --- 図の作成と保存 ---
                obj.CorrBootstrap_H(targetDataA, targetDataB, dataSpecA.SetName, dataSpecB.SetName, ...
            ...
            elseif options.Mode == 2
                ...
                % --- 図の作成と保存 ---
                obj.CorrBootstrap_HM(targetDataA, targetDataB, dataSpecA.SetName, dataSpecB.SetName, ...
                    options.Bootstrap, graphtitle, options.Amp, options.Property);
            ...
            elseif options.Mode == 3
                ...
                % --- 図の作成と保存 ---
                obj.CorrBootstrap_HS(targetDataA, targetDataB, dataSpecA.SetName, dataSpecB.SetName, ...
                    options.Bootstrap, graphtitle, options.Amp, options.Property);

                fprintf('プロットの作成が完了しました。\n');
            elseif options.Mode == 4               
                % --- 図の作成と保存 ---
                obj.CorrBootstrap_HMS(targetDataA,targetDataB, dataSpecA.SetName, dataSpecB.SetName,...
            ...
            try
                ...
                for mat = 1:size(dataA,2)
                    ...
                    PlotScatter_ver1(dataA(:,mat), dataB(:,mat), ...
                               sprintf('%s-%s\n%s', nameA, property,string(obj.MatNames3(mat))), ...
                               sprintf('%s-%s', nameB, property), ...
                               titleStr, hdrData, amp, predictionDimention);
                end
            ...
            try
                ...
                for shape = 1:size(dataA,2)
                    ...
                    PlotScatter_ver1(dataA(:,shape), dataB(:,shape), ...
                               sprintf('%s-%s\n%s', nameA, property,string(obj.ShapeNames(shape))), ...
                               sprintf('%s-%s', nameB, property), ...
                               titleStr, hdrData, amp, predictionDimention);
                end
